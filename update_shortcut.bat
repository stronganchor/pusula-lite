...
echo.
powershell -nop -c "$pythonw='%PYTHONW_PATH%'; $mainPy='%cd%\main.py'; $workDir='%cd%'; $s=New-Object -ComObject WScript.Shell; $sc=$s.CreateShortcut([Environment]::GetFolderPath('Desktop')+'\Pusula Lite.lnk'); $sc.TargetPath=$pythonw; $sc.Arguments=\"`\"$mainPy`\"\"; $sc.WorkingDirectory=$workDir; $sc.IconLocation='C:\Windows\py.exe,0'; $sc.Save();"
